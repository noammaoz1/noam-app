    ...
    <div className="content">
      <form className={styles.loginForm} action={signIn}>
        ...
        <label htmlFor="password">
          Password <input type="password" name="password" placeholder="••••••••" autoComplete="on" required />
        </label>

        <button>Log In</button>
        <button formAction={signUp}>Sign Up</button>
        {searchParams?.message && <p className={styles.errorMessage}>{searchParams.message}</p>}
        <GoogleLoginButton />
      </form>
    </div>
  );
